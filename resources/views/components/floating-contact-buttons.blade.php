<div
    x-data="{ nearFooter: false }"
    x-init="
        const footer = document.querySelector('footer');
        if (footer) {
            const observer = new IntersectionObserver(
                (entries) => { nearFooter = entries[0].isIntersecting; },
                { rootMargin: '0px 0px -10px 0px', threshold: 0 }
            );
            observer.observe(footer);
        }
    "
    class="fixed z-40 flex flex-col gap-3 bottom-6 right-6 transition-all duration-300"
    :class="nearFooter ? 'opacity-0 translate-y-2 pointer-events-none' : 'opacity-100 translate-y-0'"
>
    {{-- Bouton WhatsApp --}}
    ...
    {{-- Bouton Appel Direct --}}
    <a href="tel:+237640170487" aria-label="{{ app()->getLocale() === 'fr' ? 'Téléphone' : 'Phone' }}"
